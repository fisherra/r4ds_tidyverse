...
Strings are simply arrays of characters in your dataset. Strings can contain numbers, letters, special symbols; really anything you decide to stick between two quotation marks. The tidyverse's stringr function allows you to manipulate these strings with the help of regular expressions. While I won't be explaning regular expressions in this post, I'll link to some useful resources to help you learn to use them. But before we do anything, we have to load the ever-useful tidyverse. 
...
The easiest stringr function is `str_length()`. `str_length()` measures the number of characters in the it's primary arguement. There are 23 letters and 6 spaces, making a total of 29 characters in `string_a`. 
...
Another potentially useful function is substrings, `str_sub`. Substrings extracts and replaces substrings from a character vector, typically taking 3 input arguements. The first is your character vector, the second is where to start the substitution, and the third arguement is where to end the substitution.  
...
Stringr really becomes useful when you use regular expressions to help it sort throgh massive amounts of character vectors. RStudio's stringr [cheatsheet](https://github.com/rstudio/cheatsheets/blob/master/strings.pdf) has an excellent secton on RegEx's on its second page. If you're still not sure on how regular expressions work, check out [this](https://stringr.tidyverse.org/articles/regular-expressions.html) arcticle on the stringr website. 

<br>

### Pattern Matching with Stringr

The vignette associated with stringr has wonderful examples on how to use stringrs pattern matching functions; [check it out](https://cran.r-project.org/web/packages/stringr/vignettes/stringr.html) for great examples of all the pattern matching functions and more. 

The pattern matching functions all work in a similar way, so I won't be giving examples for each of them. All of the functions take a targeted character vector as their first arguement. The regular expression that describes the pattern you're searching for is the second arguement. Things like separators and other options come last. Here's an overview of the pattern matching functions and what they do.
...
In order to demonstrate two of my favorite pattern matching functions, I'll create some data to sift through. This data comes from the stringr vinette. 
...
String replace all has obvious uses; it's important to note that `str_replace_all` takes a third arguement defining the replacement string. 
```{r}
...
The rest of the pattern matching functions work in a similiar fashion. Sometimes stringr's usefulness is not very obvious, but that's what makes programming fun - finding creative ways to impliment what you thought was a useless function to solve a useful problem. 
...
String pad will "pad" vectors that are shorter than your desirable length with whitespace either side of the string. If your string is already greater than or equal to the desired length, it remains uneffected. `str_pad` will never shorten a vector. 
...
```{r}
# trim the whitespace returning the strings to orginal size
...
This example is directly from the stringr vinette - we've got one long string telling a jabberwocky story. We want to retain the story as one string, but break it into multiple similiar-length lines to increase accessibility. `str_wrap` to the rescue. 
...
And that's it for Unpacking the Tidyverse - stringr! There's a lot more to stringr and regular expressions, but it's probably best to learn the nitty-gritty when you actually need it. While strings arn't the most glamorous, they're prolific in many dataset and it's useful to know how to manipulate them.
...
- [Stringr website tutorial](https://stringr.tidyverse.org/articles/regular-expressions.html)
- [Stringr Cheat Sheet](https://github.com/rstudio/cheatsheets/blob/master/strings.pdf)
- [Stringr Vinette](https://cran.r-project.org/web/packages/stringr/vignettes/stringr.html)
